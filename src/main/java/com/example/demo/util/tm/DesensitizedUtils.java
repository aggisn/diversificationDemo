...
import com.example.demo.property.DesensitizedProperties;
...
public class DesensitizedUtils {
     ...
     * @return
     */
    public  String getJson(Object javaBean) {
        String json = null;
        if (null != javaBean) {
            try {
                ...
                /* 对克隆实体进行脱敏操作 */
                replace(DesensitizedObjectUtils.getAllFields(clone), clone, referenceCounter);
        ...
        if (field.getType().equals(String.class) && null != annotation && executeIsEffictiveMethod(javaBean, annotation)) {
            ...
            if (StringUtils.isNotBlank(valueStr)) {
                switch (annotation.type()) {
                    ...
                    case ADDRESS: {
                        field.set(javaBean, address(valueStr, 7));
                        break;
                    }
                    case EMAIL: {
    ...
    public  String chineseName(String fullName) {
        ...
        String name = StringUtils.left(fullName, desensitizedProperties.getA());
     ...
     * @return
     */
    public  <T> List getList(List<T> content) {
